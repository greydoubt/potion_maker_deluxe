...
from random import choice, poissonvariate
...
# Demonstrate possible potions using Ramsey theory
def find_ramsey_potions(ingredients, potions):
    n = len(ingredients)
    ...
    for i in range(1, n + 1):
        ...
        for ingredient_set in combinations(ingredients, i):
            if check_potion_set(potions, ingredient_set):
                return ingredient_set

    return None

def check_potion_set(potions, ingredient_set):
    for potion, ingredients in potions.items():
        if set(ingredients).isdisjoint(ingredient_set):
            return False
    return True

ingredients = [Herb, Mushroom, Root]
potions = {
    HealingPotion: {Herb},
    InvisibilityPotion: {Mushroom},
    StrengthPotion: {Root}
}

possible_ingredients = find_ramsey_potions(ingredients, potions)
if possible_ingredients:
    print(f"A set of ingredients that can brew all potions: {possible_ingredients}")
else:
    print("No set of ingredients found that can brew all potions.")
